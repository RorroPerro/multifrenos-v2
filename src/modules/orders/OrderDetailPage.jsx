...
import { Car, ArrowLeft, Fuel, Gauge, FileText, Plus, Trash2, Printer, Loader2, Clock, CheckCircle, Save, MessageCircle, Camera, Image as ImageIcon, X, ClipboardCheck, ChevronDown, ChevronUp, Receipt, Search, Box, Settings2, Wrench, AlertCircle, CreditCard, Banknote, DollarSign } from 'lucide-react'
...
export default function OrderDetailPage() {
  ...
  // --- NUEVO ESTADO: CONTROL DEL BUSCADOR DE SERVICIOS ---
  ...
  async function updatePaymentStatus(newStatus, newMethod = null) {
    if (!confirm(`¿Marcar como ${newStatus}?`)) return
    setIsUpdatingPayment(true)

    const updateData = {
      estado_pago: newStatus,
      metodo_pago: newMethod || order.metodo_pago
    }

    const { error } = await supabase.from('ordenes').update(updateData).eq('id', order.id)
  ...
  // --- NUEVA FUNCIÓN: CAMBIAR ESTADO DE LA ORDEN ---
  async function updateOrderStatus(newStatus) {
    if (!confirm(`¿Mover la orden a estado: ${newStatus}?`)) return

    const { error } = await supabase.from('ordenes').update({ estado: newStatus }).eq('id', order.id)
    if (!error) {
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 hidden sm:block">Etapa:</span>
          <select
            value={order.estado}
            onChange={(e) => updateOrderStatus(e.target.value)}
            className={`px-4 py-2 rounded-lg font-bold border text-sm uppercase tracking-wider outline-none cursor-pointer appearance-none text-center shadow-sm transition-colors ${order.estado === 'Agendado' ? 'bg-indigo-50 text-indigo-700 border-indigo-200 hover:bg-indigo-100' :
                order.estado === 'Recibido' ? 'bg-blue-50 text-blue-700 border-blue-200 hover:bg-blue-100' :
                  order.estado === 'En Proceso' ? 'bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-100' :
                    order.estado === 'Finalizado' ? 'bg-green-50 text-green-700 border-green-200 hover:bg-green-100' :
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            <div className="flex justify-between items-center mb-4 border-b pb-3">
              <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2">
                <Clock className="w-5 h-5 text-slate-400" /> Presupuesto / Cargos
              </h3>
              <button
              ...
              >
                {isServiceMenuOpen ? <ChevronUp className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
                {isServiceMenuOpen ? 'Ocultar Catálogo' : 'Añadir Servicio'}
              </button>
            </div>

            {/* Buscador de Cargos (AHORA ES COLAPSABLE) */}
            {isServiceMenuOpen && (
                  ...
                  {addItemType === 'servicio' && (
                    <div className="flex gap-2 relative z-20">
                      <div className="relative flex-1">
                        ...
                        <input placeholder="Buscar servicio base (ej: Afinamiento)..." className="w-full pl-10 p-2.5 border border-blue-200 rounded-lg texto-sm outline-none focus:ring-2 focus:ring-blue-500 bg-white" value={selectedService ? selectedService.nombre : searchServiceTerm} onChange={e => { setSearchServiceTerm(e.target.value); setSelectedService(null); }} />
            ...
            {/* --- PIE FINANCIERO ESTILO PÍLDORAS (NUEVO DISEÑO) --- */}
            {activeCarItems.length > 0 && (() => {
              const subtotalAuto = activeCarItems.reduce((s, i) => s + i.total_linea, 0);
              const impuestoAuto = order.incluye_iva ? Math.round(subtotalAuto * 0.19) : 0;
              const totalAuto = subtotalAuto + impuestoAuto;

              return (
                <div className="mt-6 pt-6 border-t border-slate-200 flex flex-wrap items-end justify-end gap-3 sm:gap-5">

                  <div className="flex flex-col">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 ml-2">Subtotal</span>
                    <div className="px-5 py-2.5 sm:py-3 border border-slate-300 rounded-full text-slate-600 font-mono text-base sm:text-lg font-medium bg-white">
                      {money(subtotalAuto)}
                    </div>
                  </div>

                  <div className="flex flex-col">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 ml-2">Impuesto</span>
                    <div className="pl-4 pr-2 py-2 sm:py-2.5 border border-slate-300 rounded-full bg-white flex items-center gap-3">
                      <span className={`font-mono text-base sm:text-lg font-medium ${order.incluye_iva ? 'text-slate-600' : 'text-slate-300'}`}>
                        {order.incluye_iva ? money(impuestoAuto) : '$ 0'}
                      </span>
                      {/* Botón Circular Camuflado para el IVA */}
                      <button
                        onClick={toggleIva}
                        className={`w-7 h-7 rounded-full border flex items-center justify-center transition-colors outline-none focus:ring-2 focus:ring-slate-200 ${order.incluye_iva ? 'bg-slate-100 border-slate-300 hover:bg-slate-200' : 'bg-transparent border-slate-200 hover:bg-slate-50'}`}
                        title={order.incluye_iva ? "Quitar Impuesto" : "Añadir Impuesto"}
                      >
                        <div className={`w-3.5 h-3.5 rounded-full transition-all ${order.incluye_iva ? 'bg-slate-400' : 'bg-slate-200'}`}></div>
                      </button>
                    </div>
                  </div>

                  <div className="flex flex-col w-full sm:w-auto mt-2 sm:mt-0">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 ml-2">Total</span>
                    <div className="px-6 py-2.5 sm:py-3 border-2 border-blue-400 rounded-full text-blue-600 font-mono text-xl sm:text-2xl font-black bg-blue-50/50 flex justify-center">
                      {money(totalAuto)}
                    </div>
                  </div>
      ...
      {activeTab === 'resumen' && (
        <div className="space-y-6 animate-fade-in max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* LADO IZQUIERDO: EL MONTO */}
            <div className="bg-white p-8 sm:p-10 rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center text-center relative overflow-hidden">
            ...
            {/* LADO DERECHO: GESTIÓN DE PAGO (LA NUEVA FUNCIÓN) */}
            <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-200 flex flex-col justify-center">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4 block">Estado Financiero</h3>

              {order.estado_pago === 'Pagado' ? (
                <div className="bg-green-50 border-2 border-green-200 p-5 rounded-xl flex flex-col items-center justify-center h-full gap-2 relative group overflow-hidden">
                  <div className="absolute -right-4 -bottom-4 bg-green-200/50 w-24 h-24 rounded-full blur-xl group-hover:bg-green-300/50 transition-colors"></div>
                  <CheckCircle className="w-10 h-10 text-green-500 relative z-10" />
                  <p className="font-black text-green-700 text-xl relative z-10">¡Pagado!</p>
                  {order.metodo_pago && (
                    <p className="text-xs font-bold text-green-600 bg-white px-3 py-1 rounded-full uppercase tracking-wider relative z-10 shadow-sm">
                      Vía {order.metodo_pago}
                    </p>
                  )}
                  <button
                    onClick={() => updatePaymentStatus('Pendiente')}
                    disabled={isUpdatingPayment}
                    className="mt-3 text-[10px] text-slate-400 hover:text-red-500 underline font-bold relative z-10"
                  >
                    Deshacer y marcar como Pendiente
                  </button>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="bg-orange-50 text-orange-600 border border-orange-200 p-3 rounded-lg text-center font-bold text-sm animate-pulse-slow">
                    Cobro Pendiente
                  </div>

                  {/* GRID DE 3 BOTONES DE PAGO */}
                  <div className="grid grid-cols-3 gap-2 sm:gap-3 pt-2">
                    <button
                      onClick={() => updatePaymentStatus('Pagado', 'Efectivo')}
                      disabled={isUpdatingPayment}
                      className="bg-slate-50 hover:bg-green-50 border border-slate-200 hover:border-green-300 text-slate-600 hover:text-green-700 p-3 sm:p-4 rounded-xl flex flex-col items-center justify-center gap-2 transition-all group"
                    >
                      <Banknote className="w-5 sm:w-6 h-5 sm:h-6 group-hover:scale-110 transition-transform" />
                      <span className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-center">Efectivo</span>
                    </button>

                    <button
                      onClick={() => updatePaymentStatus('Pagado', 'Transferencia')}
                      disabled={isUpdatingPayment}
                      className="bg-slate-50 hover:bg-blue-50 border border-slate-200 hover:border-blue-300 text-slate-600 hover:text-blue-700 p-3 sm:p-4 rounded-xl flex flex-col items-center justify-center gap-2 transition-all group"
                    >
                      <Banknote className="w-5 sm:w-6 h-5 sm:h-6 group-hover:scale-110 transition-transform" />
                      <span className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-center">Transf.</span>
                    </button>

                    <button
                      onClick={() => updatePaymentStatus('Pagado', 'Tarjeta')}
                      disabled={isUpdatingPayment}
                      className="bg-slate-50 hover:bg-purple-50 border border-slate-200 hover:border-purple-300 text-slate-600 hover:text-purple-700 p-3 sm:p-4 rounded-xl flex flex-col items-center justify-center gap-2 transition-all group"
                    >
                      <CreditCard className="w-5 sm:w-6 h-5 sm:h-6 group-hover:scale-110 transition-transform" />
                      <span className="text-[10px] sm:text-xs font-bold uppercase tracking-widest text-center">Tarjeta</span>
                    </button>
                  </div>
                </div>
              )}
          ...
          {/* BOTÓN WHATSAPP */}
          <div className="flex flex-col sm:flex-row gap-4 justify-center w-full relative z-10 pt-2">
